...
func init() {
	android.RegisterModuleType("lean_generator", GeneratorFactory)
...
func (g *Module) GenerateAndroidBuildActions(ctx android.ModuleContext) {
	...
	if depRoot == "" {
		...
		depRoot = leanExpandVariables(ctx, depRoot)
	}

	// Glob dep_files property
	for _, dep_file := range g.properties.Dep_files {
		dep_file = leanExpandVariables(ctx, dep_file)
	...
	cmd := leanExpandVariables(ctx, String(g.properties.Cmd))
